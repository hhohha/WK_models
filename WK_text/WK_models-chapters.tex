...
\chapter{Implementation of the state space search}
...
There is a lot of parameters that could be tested and analysed. How efficient are the various heuristics (both pruning and rule preference) for different grammars. What inputs lengths are answered withing a reasonable time frame? Or more generally -- what is the relation between input length and time to get the answer? What are the memory requirements? What is the difference between input strings that are accepted and strings that are rejected? Is there a difference between some inputs of the same lengths?

In order to analyse these questions, I have decided to test the state space search in the following four stages.
\begin{enumerate}
  \item{Comparison of the heuristics and analysis of their efficiency.}
...
\subsection{Comparison of the heuristics efficiency}
The space state search algorithm contains five different tests of pruning the state tree (which can and usually do work at the same time) and five rule precedence heuristics (of which only one is active). In order to compare their effectiveness I used the script \textit{heuristic\_compare.py} which runs two tests for each of the 40 grammars -- with one input that will be accepted and one that will be rejected. The input strings have been chosen to have suitable lengths, so that the computation is finished (at least with some heuristics) in a reasonable time -- specifically within the time limit of seven seconds, but also to last some measurable amount of time.

Each of the 80 tests is run several times with different settings of the heurisctics. First, the rule precedence heuristics are tested -- all pruning options are turned on and the same test is executed with all the available rule precedences. Next, the test is run again, this time the rule precedence is fixed and the pruning options are turned off one at a time.

The results that are produces by each tests are:
\begin{itemize}
  \item{time of the computation}
  \item{number of states that have been analysed and maximum number of states that were in the queue, this is useful for memory requirement analysis}
  \item{how many times which prunings were used}
\end{itemize}

Each test prints out the results in a table similar to LINK.
...
\begin{tabular}{ |l|l|l|l|l|  }
  ...
  \hline
  Strategy & Time & States Q+C & States pruned & Accepted \\
  \hline
  no heuristic & $4.0791$ & $8675 + 14280$ & $0, 75, 0, 0, 397$ & True \\
  prefer less non-terminals & $0.6923$ & $798 + 1201$ & 0, 3, 0, 0, 400 & True \\
  non terms distance & $0.6936$ & $798 + 1201$ & $0, 3, 0, 0, 400$ & True  \\
  prefer prefix matching goal & $0.7756$ & $798 + 1201$ & $0, 3, 0, 0, 400$ & True \\
  individual terms matching goal & $0.7691$ & $798 + 1201$ & $0, 3, 0, 0, 400$ & True \\
  matching goal + prefer terminals & $0.7717$ & $798 + 1201$ & $0, 3, 0, 0, 400$ & True \\
  \hline
  Pruning & Time & States Q+C & States pruned & Accepted \\
  \hline
  ALL ON & $0.771$ & $798 + 1201$ & $0, 3, 0, 0, 400$ & True \\
  prune\_check\_strands\_len OFF & $0.7731$ & $798 + 1201$ & $0, 3, 0, 0, 400$ & True \\
  prune\_check\_total\_len OFF & $0.7734$ & $801 + 1201$ & $0, 0, 0, 0, 400$ & True \\
  prune\_check\_word\_start OFF & $0.7719$ & $798 + 1201$ & $0, 3, 0, 0, 400$ & True \\
  prune\_check\_relation OFF & $0.645$ & $798 + 1201$ & $0, 3, 0, 0, 400$ & True \\
  prune\_check\_regex OFF & $0.3673$ & $798 + 1601$ & $0, 3, 0, 0, 0$ & True \\
  ALL OFF & $0.2243$ & $801 + 1601$ & $0, 0, 0, 0, 0$ & True\\
  \hline
\end{tabular}


\subsection{Analysis of the time and memory complexity}
