...
def main():
	...
	t.var_inputs_test(g5, inputs, True)
	...
	s2 = 20*'(' + '(0+1)*' + 20*')*'
	s3 = '0+'*40 + '1'
	regex = s1.replace('(', 'o').replace(')', 'c').replace('+', 'p').replace('*', 's')

	inputs = [
		(f's1+s1 while s1={s1}', (s1 + '+' + s1).replace('(', 'o').replace(')', 'c').replace('+', 'p').replace('*', 's'), True),
		('(((...(((0.1)*)*...)*)', s2.replace('(', 'o').replace(')', 'c').replace('+', 'p').replace('*', 's'), True),
		('0+0+...0+0+1', s3.replace('(', 'o').replace(')', 'c').replace('+', 'p').replace('*', 's'), True),
		('+0+0+...0+0+1', ('+'+s3).replace('(', 'o').replace(')', 'c').replace('+', 'p').replace('*', 's'), False),
		('0+0+...0+0+1+', (s3+'+').replace('(', 'o').replace(')', 'c').replace('+', 'p').replace('*', 's'), False),
		('0+0+...0++0+...0+0+1',  (s3[:40] + '+' + s3[40:]).replace('(', 'o').replace(')', 'c').replace('+', 'p').replace('*', 's'), False)
	]
	t.var_inputs_test(g10, inputs)
